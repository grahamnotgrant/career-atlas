...
1. Read fresh settings and the previous `scout-roles` session. Note the checkpoint in `DATA/scout/checkpoint.json`; the next poll starts there. On a first run, `npm run scout -- seed` builds the board list from every stored opportunity URL.
...
6. Grow the board list when time allows. Find companies not yet on it by any source available to you: a web search limited to `jobs.ashbyhq.com`, `job-boards.greenhouse.io` or `jobs.lever.co`; an authorized job-search connector; a browser; the monthly Hacker News hiring thread; an employer careers page. For each new company, `npm run scout -- add <ats> <slug> "<Company>"`. The next poll covers it automatically. Aggregator listings (Indeed, Dice, LinkedIn, ZipRecruiter) are leads to a board, not records: do not store an aggregator link as the opportunity URL when the employer page exists.
