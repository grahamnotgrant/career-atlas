...
1. Read fresh records before scouting or claiming. `../scout-roles/SKILL.md` and `npm run scout` supply discovered roles from employer boards; use them before searching elsewhere. Capture employer, original title, canonical job URL, full description, location, compensation and source date. Screen duties against evidence and constraints. Preserve discovered and rejected-for-fit records without counting them as applications.
2. Normalize employer/job identity and check company history. Distinct jobs at the same company are separate records; duplicate source listings of one job are not. Store the role as an opportunity and `triage` it: `top` for a strong fit or high pay, `standard` otherwise, `skip` when it fails the grant. A hold set by the user stops work on that role until released.
3. Verify a user authorization grant covers the role, pay, location and exclusions. Check the company's recorded application limit; `claim` refuses a company at its cap and names the next eligible date, so choose another role rather than waiting. Claim the eligible job through the API. Retain owner, expiry and fencing token. If another agent owns it, choose another job.
4. Read the full description. Tailor a resume and answers from the approved template for that family: adjust emphasis and wording, add no claim outside the template and the facts file, and run `../stop-slop/SKILL.md` on the result. Preserve exact files, hashes and source mappings. Check facts, extraction, reading order and writing. Record the checks with `vet`; a failed check ends the attempt.
